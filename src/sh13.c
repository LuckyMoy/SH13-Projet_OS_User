...
// ---- Fonction du thread de réception des messages serveur ----
...
// ---- fonction d'envoi des messages vers le serveur
...
// ---- main ----
// Connection, Gestion des infos recues du serveur, des choix du joueurs et de l'affichage
int main(int argc, char ** argv) {
	...
	// Initialisation de la fenetre et du moteur de rendu SDL
	...
	deck[0] = IMG_Load("resources/img/SH13_0.png");
	deck[1] = IMG_Load("resources/img/SH13_1.png");
	deck[2] = IMG_Load("resources/img/SH13_2.png");
	deck[3] = IMG_Load("resources/img/SH13_3.png");
	deck[4] = IMG_Load("resources/img/SH13_4.png");
	deck[5] = IMG_Load("resources/img/SH13_5.png");
	deck[6] = IMG_Load("resources/img/SH13_6.png");
	deck[7] = IMG_Load("resources/img/SH13_7.png");
	deck[8] = IMG_Load("resources/img/SH13_8.png");
	deck[9] = IMG_Load("resources/img/SH13_9.png");
	deck[10] = IMG_Load("resources/img/SH13_10.png");
	deck[11] = IMG_Load("resources/img/SH13_11.png");
	deck[12] = IMG_Load("resources/img/SH13_12.png");

	objet[0] = IMG_Load("resources/img/SH13_pipe_120x120.png");
	objet[1] = IMG_Load("resources/img/SH13_ampoule_120x120.png");
	objet[2] = IMG_Load("resources/img/SH13_poing_120x120.png");
	objet[3] = IMG_Load("resources/img/SH13_couronne_120x120.png");
	objet[4] = IMG_Load("resources/img/SH13_carnet_120x120.png");
	objet[5] = IMG_Load("resources/img/SH13_collier_120x120.png");
	objet[6] = IMG_Load("resources/img/SH13_oeil_120x120.png");
	objet[7] = IMG_Load("resources/img/SH13_crane_120x120.png");

	gobutton = IMG_Load("resources/img/gobutton.png");
	connectbutton = IMG_Load("resources/img/connectbutton.png");
    ...
    TTF_Font* Sans = TTF_OpenFont("resources/sans.ttf", 15); 
    printf("Sans=%p\n",Sans);
    ...
    while (!quit) {
		if (SDL_PollEvent(&event)) {
        	...
        	switch (event.type)
        	{
				...
				case  SDL_MOUSEBUTTONDOWN:
					...
					// appui sur le boutton connect
					if ((mx<200) && (my<50) && (connectEnabled==1)) {

						// message C: conection au serveur
					...
					// Sélection d'un coupable
					else if ((mx>=100) && (mx<250) && (my>=350) && (my<740)) {
						joueurSel=-1;
						objetSel=-1;
						guiltSel=(my-350)/30;
						if(guiltGuessOut[guiltSel])
							guiltSel=-1;
					} 
					// Sélction d'un personage dans la grille (croix)
					else if ((mx>=250) && (mx<300) && (my>=350) && (my<740)) {
						int ind=(my-350)/30;
						guiltGuess[ind]=1-guiltGuess[ind];
					} 
					// Appui sur le boutton go
					else if ((mx>=500) && (mx<700) && (my>=350) && (my<450) && (goEnabled==1)) {
						printf("go! joueur=%d objet=%d guilt=%d\n",joueurSel, objetSel, guiltSel);

						// Seul un personnage est selectionné : Accusation
						if (guiltSel!=-1) {
							// Message G: accusation
							sprintf(sendBuffer,"G %d %d",gId, guiltSel);
							sendMessageToServer(gServerIpAddress, gServerPort, sendBuffer);
							guessed = 1;
						} 
						
						// Un joueur et un objet sont seléctionnés : Enquete 2
						...
						// Un objet seulement est selectionné : Enquete 1
						else if ((objetSel!=-1)) {
							// Message S: enquete 1
					...
					// Clic sur rien : on rince les selections
					else {
						joueurSel=-1;
						objetSel=-1;
						guiltSel=-1;
        ...
        if (synchro==1) {
			...
			switch (gbuffer[0]) {
				...
				case 'M':
					...
					if(guessed && currentPlayer == gId && partieTerminee==0) {
						sprintf(sendBuffer, "P"); // Pass
						sendMessageToServer(gServerIpAddress, gServerPort, sendBuffer);
					} else {
						goEnabled = (currentPlayer == gId) ? 1 : 0;
					}
				...
				case 'V':
					// RAJOUTER DU CODE ICI
					sscanf(gbuffer, "V %d %d %s", &j, &o, value);
					if (strcmp(value, "*") == 0 && j!=currentPlayer)
						tableCartes[j][o] = 100;
					else
						tableCartes[j][o] = atoi(value);
					break;

				// Message A, une accusation a été faite et une carte n'est pas le tueur
				case 'A':
					sscanf(gbuffer, "A %d", &perso);
					guiltGuessOut[perso] = 1;
					break;

				// Message E, Fin de partie
				case 'E':
					sscanf(gbuffer, "E %d %d", &gagnant, &tueur);
					partieTerminee = 1;
					goEnabled = 0;
					break;
			}
		...
		// ---- Graphismes : dessin de l'inteface graphique ----
        ...
        for(int i=0;i<13;i++) {
			if(guiltGuessOut[i]) {
				SDL_SetRenderDrawColor(renderer, 180, 255, 128, 128);
				SDL_Rect rect1 = {100, 350+i*30, 150 , 30}; 
				SDL_RenderFillRect(renderer, &rect1);
			}
			if(partieTerminee && tueur == i) {
		...
		if(partieTerminee) {
			char buffer[200];
			if(gagnant == gId) {
				sprintf(buffer, "Partie terminee : Vous avez gagne");
			} else if(gagnant == -1) {
				sprintf(buffer, "Partie terminee : Tous les joueurs ont ete elimines");
			}
			else {
				sprintf(buffer, "Partie terminee : %s a gagne", gNames[gagnant]);
			}
